...
def upload_files_from_folder(folder_path):
    for filename in os.listdir(folder_path):
        ...
        with open(file_path, "rb") as file:
            ...
            print(f"{filename} Successfully Uploaded to Database")
...
def download_and_convert_files(local_folder_path):
    ...
    for document in cursor:
        ...
        try:
            ...
            print(f"{filename} Successfully Downloaded From Database")
        except Exception as e:
            print(f"{filename} Error {str(e)}")

    # Call the encode_faces() method after the loop
    fr.encode_faces()
...
def submit():
    ...
    if len(mail) < 6 or len(mail) > 30:
        print("Invalid input. Email must be between 6 and 30 characters.")
    else:
        window2.after(1000, window2.quit)

# Tuşa bağlı olarak submit fonksiyonunu çağırmak için binding ekleniyor
        ...
        while True:
            ...
            if self.process_current_frame:
                ...
                for face_encoding in self.face_encodings:
                    ...
                    if name == 'Unknown':
                        intruderDB()
                        window = pygetwindow.getWindowsWithTitle("Face Recognition")[0]
                        left, top, width, height = window.left, window.top, window.width, window.height
                        screenshot = pyautogui.screenshot(region=(left, top, width, height))
                        screenshot.save("Stranger at Door.png")
...
if __name__ == '__main__':
    ...
    button5 = customtkinter.CTkButton(window2, text="Submit", command=lambda:submit())
    button5.pack()
    ...
    button3 = customtkinter.CTkButton(window, text="Upload Images to Database", command=lambda:upload_files_from_folder(folder_path))
    button3.pack(pady=20)
            
    button4 = customtkinter.CTkButton(window, text="Download Images from Database", command=lambda:download_and_convert_files(folder_path))
